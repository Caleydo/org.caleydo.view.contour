 ...
 *
 */
public class ActivitySummaryItemFactoryCreator implements ISummaryItemFactoryCreator {
		...
		protected class BoxPlot extends PickableGLElement {
			...
			protected void renderImpl(GLGraphics g, float w, float h) {
...

				{
					...
					g.color(color).fillRect(firstQuantrileBoundary, y, thirdQuantrileBoundary - firstQuantrileBoundary,
							hi);
		 ...
		 */
			...
			protected void renderImpl(GLGraphics g, float w, float h) {
				...
				for (Object elementID : filteredElementIDs) {
					...
					g.color(Color.GRAY).drawLine(2 + normalizedValue * (w - 4), h / 2f, 2 + normalizedValue * (w - 4),
							h);
				}
				g.lineWidth(1);
