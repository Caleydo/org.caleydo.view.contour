...
import org.caleydo.view.relationshipexplorer.ui.column.item.factory.impl.HTIMutationItemConfigurationAddon.HTIMutationItemFactoryCreator.EColumn;
 ...
 *
 */
public class HTIMutationItemConfigurationAddon implements IItemFactoryConfigurationAddon {

	public static class HTIMutationItemFactoryCreator implements IItemFactoryCreator {

		protected static final URL PLOT_ICON = HTIMutationItemFactory.class
		...
		public static class HTIMutationItemFactory implements IItemFactory {
			...
			public HTIMutationItemFactory(TabularDataCollection collection, ConTourElement contour) {
				...
				final IDType recordIDType = dataDomain.getOppositeIDType(collection.getDimensionPerspective()
						.getIdType());
			...
			public GLElement createItem(Object elementID) {
				...
				GLElementContainer container = new GLElementContainer(new GLSizeRestrictiveFlowLayout2(true, 3,
			...
			protected GLElement createTextElement(ATableBasedDataDomain dataDomain, IDType recordIDType, int elementID,
				...
				String text = (String) dataDomain.getRaw(recordIDType, elementID,
			...
			protected GLElement createCategoryElement(ATableBasedDataDomain dataDomain, IDType recordIDType,
					int elementID, EColumn column) {
				Color color = new Color(dataDomain.getColor(recordIDType, elementID,
						HTIMutationItemFactory.this.collection.getDimensionPerspective().getIdType(),
						columnToIndex.get(column)));
			...
			protected GLElement createNumericalElement(ATableBasedDataDomain dataDomain, IDType recordIDType,
					int elementID, EColumn column) {

				// float normalizedValue = dataDomain
				// .getNormalizedValue(recordIDType, elementID, HTIMutationItemFactory.this.collection
				// .getDimensionPerspective().getIdType(), columnToIndex.get(column));
				Number value = (Number) dataDomain.getRaw(recordIDType, elementID,
						HTIMutationItemFactory.this.collection.getDimensionPerspective().getIdType(),
						columnToIndex.get(column));
				float normalizedValue = value.intValue() == Integer.MIN_VALUE ? Float.NaN : (float) columnToNormalize
						.get(column).apply(value.doubleValue());
			...
			protected GLElement createCOSMICElement(GLElementContainer container, Object elementID,
				...
				final Set<Object> ids = idMappingManager
						.getIDAsSet(recordIDType, IDType.getIDType("COSMIC"), elementID);

				int numElements = 0;
				GLElementContainer element = new GLElementContainer(new GLSizeRestrictiveFlowLayout2(true, 1,
						GLPadding.ZERO));
				element.setMinSizeProvider(GLMinSizeProviders.createHorizontalFlowMinSizeProvider(element, 1,
						GLPadding.ZERO));
			...
			protected GLElement createPositionElement(Object elementID, ATableBasedDataDomain dataDomain,
					IDType recordIDType) {
			...
			public void update() {
				...
				for (Object elementID : collection.getFilteredElementIDs()) {
					...
					IDMappingManager idMappingManager = IDMappingManagerRegistry.get()
							.getIDMappingManager(recordIDType);
					final Set<Object> ids = idMappingManager.getIDAsSet(recordIDType, IDType.getIDType("COSMIC"),
							elementID);
		...
		public IItemFactory create(IEntityCollection collection, IColumnModel column, ConTourElement contour) {
			...
			return new HTIMutationItemFactory((TabularDataCollection) collection, contour);
		}
